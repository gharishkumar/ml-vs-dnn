...
def logistic_regression(X, y, class_weight=None):
    ...
    Returns:
        Predictions and loss tensors.
    """
    with tf.compat.v1.variable_scope('logistic_regression'):
        tf.summary.histogram('logistic_regression.X', X)
        tf.summary.histogram('logistic_regression.y', y)
        weights = tf.compat.v1.get_variable('weights', [X.get_shape()[1],
                                              y.get_shape()[-1]])
        bias = tf.compat.v1.get_variable('bias', [y.get_shape()[-1]])
        tf.summary.histogram('logistic_regression.weights', weights)
        tf.summary.histogram('logistic_regression.bias', bias)
        # If no class weight provided, try to retrieve one from pre-defined
        # tensor name in the graph.
        if not class_weight:
            try:
                class_weight = tf.compat.v1.get_default_graph().get_tensor_by_name('class_weight:0')
            except KeyError:
                pass
