...
class TensorFlowEstimator(BaseEstimator):
    ...
    def _setup_training(self):
        ...
        with self._graph.as_default():
            tf.compat.v1.set_random_seed(self.tf_random_seed)
            ...
            self._inp = tf.compat.v1.placeholder(
                tf.as_dtype(self._data_feeder.input_dtype), input_shape,
                name="input")
            self._out = tf.compat.v1.placeholder(
            ...
            if self._data_feeder.input_dtype in (np.float32, np.float64):
                tf.summary.histogram("X", self._inp)
            if self._data_feeder.output_dtype in (np.float32, np.float64):
                tf.summary.histogram("y", self._out)
            ...
            tf.summary.scalar("loss", self._model_loss)
            ...
            self._session = tf.compat.v1.Session(self.tf_master,
    ...
    def _setup_summary_writer(self, logdir):
        ...
        self._summary_writer = tf.compat.v1.train.SummaryWriter(
